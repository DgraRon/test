...
public class PriorityController : Controller
{
    ...
    public PriorityController(ApplicationDbContext context) => _context = context;

    public IActionResult Index() =>
        View(_context.Priorities.Where(x => !x.Deleted).ToList());
    ...
    public IActionResult Create(Priority model, string? returnUrl)
    {
    ...
    {
        var item = _context.Priorities.Find(id);
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public IActionResult Edit(int id, Priority model, string? returnUrl)
    {
        ...
        var original = _context.Priorities.Find(id);
    ...
    public IActionResult CreateQuick([FromForm] string name)
    {
        ...
        if (string.IsNullOrWhiteSpace(name))
            return BadRequest(new { message = "กรุณากรอกชื่อ Priority" });

        var exists = _context.Priorities
